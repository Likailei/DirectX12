...
void Game::LoadPipeline() {
    ...
    // Descriptor heaps.
    {
        ...
        cbvSrvHeapDesc.NumDescriptors = NUM_TEXTURE + FrameCount * CHUNKCNT;
...
void Game::LoadAssets() {
    ...
    {
        for (unsigned ci = 0; ci < CHUNKCNT; ++ci) {
            Mesh m = m_chunks[ci].GetChunkMesh();
            UINT vSize = m.v.size() * sizeof(Vertex);
            UINT iSize = m.i.size() * sizeof(UINT);
            ...
            ThrowIfFailed(m_device->CreateCommittedResource(
                ...
                nullptr,
                IID_PPV_ARGS(&m_vertexBuffers[ci])));
            ...
            ThrowIfFailed(m_device->CreateCommittedResource(
                ...
                nullptr,
                IID_PPV_ARGS(&vertexBufferUploadHeaps[ci])));

            D3D12_SUBRESOURCE_DATA vertexData = {};
            vertexData.pData = (UINT8*)(m.v.data());
            vertexData.RowPitch = vSize;
            vertexData.SlicePitch = vertexData.RowPitch;

            UpdateSubresources<1>(m_commandList.Get(), m_vertexBuffers[ci].Get(), vertexBufferUploadHeaps[ci].Get(), 0, 0, 1, &vertexData);
            m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_vertexBuffers[ci].Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER));

            // Initialize the vertex buffer view.
            m_vertexBufferViews[ci].BufferLocation = m_vertexBuffers[ci]->GetGPUVirtualAddress();
            m_vertexBufferViews[ci].StrideInBytes = sizeof(Vertex);
            m_vertexBufferViews[ci].SizeInBytes = vSize;
            ...
            ThrowIfFailed(m_device->CreateCommittedResource(
                ...
                D3D12_RESOURCE_STATE_COPY_DEST,
                nullptr,
                IID_PPV_ARGS(&m_indexBuffers[ci])));
            ...
            ThrowIfFailed(m_device->CreateCommittedResource(
                ...
                D3D12_RESOURCE_STATE_GENERIC_READ,
                nullptr,
                IID_PPV_ARGS(&indexBufferUploadHeaps[ci])));

            D3D12_SUBRESOURCE_DATA indexData = {};
            indexData.pData = (UINT8*)(m.i.data());
            indexData.RowPitch = iSize;
            indexData.SlicePitch = indexData.RowPitch;

            UpdateSubresources<1>(m_commandList.Get(), m_indexBuffers[ci].Get(), indexBufferUploadHeaps[ci].Get(), 0, 0, 1, &indexData);
            m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_indexBuffers[ci].Get(), D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER));

            // Initialize the vertex buffer view.
            m_indexBufferViews[ci].BufferLocation = m_indexBuffers[ci]->GetGPUVirtualAddress();
            m_indexBufferViews[ci].Format = DXGI_FORMAT_R32_UINT;
            m_indexBufferViews[ci].SizeInBytes = iSize;
        }
    ...
    // Constant buffer view.
    {
        ...
        UINT bufferCount = (CHUNKCNT + cbCountInBufferStep - 1) / cbCountInBufferStep;
...
void Game::InitBundles() {
    ...
    for (UINT8 i = 0; i < FrameCount; i++) {
        ...
        for (int n = 0; n < CHUNKCNT; n++) {
            m_bundles[i]->IASetVertexBuffers(0, 1, &m_vertexBufferViews[n]);
            m_bundles[i]->IASetIndexBuffer(&m_indexBufferViews[n]);
...
void Game::OnUpdate() {
    ...
    XMMATRIX world;
    for (UINT n = 0; n < CHUNKCNT; n++) {
        XMFLOAT3 p = m_chunks[n].GetChunkPosition3f();
        world = XMMatrixTranslationFromVector(XMLoadFloat3(&p));
